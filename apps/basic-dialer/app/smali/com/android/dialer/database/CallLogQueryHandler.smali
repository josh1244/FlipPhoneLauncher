.class public Lcom/android/dialer/database/CallLogQueryHandler;
.super Lcom/android/contacts/common/database/NoNullCursorAsyncQueryHandler;
.source "CallLogQueryHandler.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/android/dialer/database/CallLogQueryHandler$Listener;,
        Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;,
        Lcom/android/dialer/database/CallLogQueryHandler$CatchingWorkerHandler;
    }
.end annotation


# static fields
.field public static final CALL_TYPE_ALL:I = -0x1

.field private static final NUM_LOGS_TO_DISPLAY:I = 0x3e8

.field private static final QUERY_CALLLOG_TOKEN:I = 0x36

.field private static final QUERY_MISSED_CALLS_UNREAD_COUNT_TOKEN:I = 0x3b

.field private static final QUERY_VOICEMAIL_STATUS_TOKEN:I = 0x39

.field private static final QUERY_VOICEMAIL_UNREAD_COUNT_TOKEN:I = 0x3a

.field private static final TAG:Ljava/lang/String; = "CallLogQueryHandler"

.field private static final UPDATE_MARK_AS_OLD_TOKEN:I = 0x37

.field private static final UPDATE_MARK_MISSED_CALL_AS_READ_TOKEN:I = 0x38


# instance fields
.field private eccTableString:Ljava/lang/String;

.field private final mBuildQueryListener:Ljava/lang/ref/WeakReference;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/lang/ref/WeakReference<",
            "Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;",
            ">;"
        }
    .end annotation
.end field

.field private final mContext:Landroid/content/Context;

.field private final mListener:Ljava/lang/ref/WeakReference;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/lang/ref/WeakReference<",
            "Lcom/android/dialer/database/CallLogQueryHandler$Listener;",
            ">;"
        }
    .end annotation
.end field

.field private final mLogLimit:I


# direct methods
.method public constructor <init>(Landroid/content/Context;Landroid/content/ContentResolver;Lcom/android/dialer/database/CallLogQueryHandler$Listener;Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;)V
    .locals 6

    const/4 v5, -0x1

    move-object v0, p0

    move-object v1, p1

    move-object v2, p2

    move-object v3, p3

    move-object v4, p4

    .line 97
    invoke-direct/range {v0 .. v5}, Lcom/android/dialer/database/CallLogQueryHandler;-><init>(Landroid/content/Context;Landroid/content/ContentResolver;Lcom/android/dialer/database/CallLogQueryHandler$Listener;Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;I)V

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Landroid/content/ContentResolver;Lcom/android/dialer/database/CallLogQueryHandler$Listener;Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;I)V
    .locals 2

    .line 102
    invoke-direct {p0, p2}, Lcom/android/contacts/common/database/NoNullCursorAsyncQueryHandler;-><init>(Landroid/content/ContentResolver;)V

    .line 103
    invoke-virtual {p1}, Landroid/content/Context;->getApplicationContext()Landroid/content/Context;

    move-result-object p2

    iput-object p2, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 104
    new-instance p2, Ljava/lang/ref/WeakReference;

    invoke-direct {p2, p3}, Ljava/lang/ref/WeakReference;-><init>(Ljava/lang/Object;)V

    iput-object p2, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mListener:Ljava/lang/ref/WeakReference;

    .line 105
    new-instance p2, Ljava/lang/ref/WeakReference;

    invoke-direct {p2, p4}, Ljava/lang/ref/WeakReference;-><init>(Ljava/lang/Object;)V

    iput-object p2, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mBuildQueryListener:Ljava/lang/ref/WeakReference;

    iput p5, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mLogLimit:I

    .line 108
    invoke-virtual {p1}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object p1

    const p2, 0x7f030003

    invoke-virtual {p1, p2}, Landroid/content/res/Resources;->getStringArray(I)[Ljava/lang/String;

    move-result-object p1

    .line 109
    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2}, Ljava/lang/StringBuilder;-><init>()V

    .line 110
    array-length p3, p1

    const/4 p4, 0x0

    :goto_0
    if-ge p4, p3, :cond_1

    aget-object p5, p1, p4

    .line 111
    invoke-virtual {p2}, Ljava/lang/StringBuilder;->length()I

    move-result v0

    if-lez v0, :cond_0

    const/16 v0, 0x2c

    .line 112
    invoke-virtual {p2, v0}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    :cond_0
    const/16 v0, 0x27

    .line 115
    invoke-virtual {p2, v0}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    move-result-object v1

    invoke-virtual {v1, p5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p5

    invoke-virtual {p5, v0}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 p4, p4, 0x1

    goto :goto_0

    .line 117
    :cond_1
    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    iput-object p1, p0, Lcom/android/dialer/database/CallLogQueryHandler;->eccTableString:Ljava/lang/String;

    return-void
.end method

.method private cancelFetch()V
    .locals 1

    const/16 v0, 0x36

    .line 282
    invoke-virtual {p0, v0}, Lcom/android/dialer/database/CallLogQueryHandler;->cancelOperation(I)V

    return-void
.end method

.method private fetchCalls(IIZLjava/lang/String;J)V
    .locals 10

    move-object v8, p0

    move v0, p2

    .line 219
    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1}, Ljava/lang/StringBuilder;-><init>()V

    .line 220
    new-instance v2, Ljava/util/ArrayList;

    invoke-direct {v2}, Ljava/util/ArrayList;-><init>()V

    const-string v3, "(type != ?)"

    .line 223
    invoke-virtual {v1, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const/4 v3, 0x6

    .line 224
    invoke-static {v3}, Ljava/lang/Integer;->toString(I)Ljava/lang/String;

    move-result-object v3

    invoke-interface {v2, v3}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    const/16 v3, 0x17

    .line 227
    invoke-static {v3}, Lcom/android/dialer/compat/SdkVersionOverride;->getSdkVersion(I)I

    move-result v4

    if-lt v4, v3, :cond_0

    const-string v3, " AND (deleted = 0)"

    .line 228
    invoke-virtual {v1, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    :cond_0
    if-eqz p3, :cond_1

    const-string v3, " AND (new = 1)"

    .line 232
    invoke-virtual {v1, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 235
    :cond_1
    invoke-static {p4}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v3

    if-eqz v3, :cond_2

    const-string v3, " AND (number NOT IN ( "

    .line 236
    invoke-virtual {v1, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    iget-object v4, v8, Lcom/android/dialer/database/CallLogQueryHandler;->eccTableString:Ljava/lang/String;

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    const-string v4, " ))"

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    goto :goto_0

    :cond_2
    const-string v3, " AND (number = \'"

    .line 238
    invoke-virtual {v1, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    move-object v4, p4

    invoke-virtual {v3, p4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    const-string v4, "\' )"

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    :goto_0
    const/4 v3, -0x1

    if-le v0, v3, :cond_3

    const-string v4, " AND (type = ?)"

    .line 242
    invoke-virtual {v1, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 243
    invoke-static {p2}, Ljava/lang/Integer;->toString(I)Ljava/lang/String;

    move-result-object v4

    invoke-interface {v2, v4}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    goto :goto_1

    :cond_3
    const-string v4, " AND NOT (type = 4)"

    .line 245
    invoke-virtual {v1, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    :goto_1
    const-wide/16 v4, 0x0

    cmp-long v4, p5, v4

    if-lez v4, :cond_4

    const-string v4, " AND (date > ?)"

    .line 250
    invoke-virtual {v1, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 251
    invoke-static/range {p5 .. p6}, Ljava/lang/Long;->toString(J)Ljava/lang/String;

    move-result-object v4

    invoke-interface {v2, v4}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_4
    const/4 v4, 0x4

    if-ne v0, v4, :cond_5

    iget-object v0, v8, Lcom/android/dialer/database/CallLogQueryHandler;->mBuildQueryListener:Ljava/lang/ref/WeakReference;

    .line 255
    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;

    if-eqz v0, :cond_5

    iget-object v4, v8, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 257
    invoke-interface {v0, v4, v1, v2}, Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;->appendOmtpVoicemailSelectionClause(Landroid/content/Context;Ljava/lang/StringBuilder;Ljava/util/List;)V

    :cond_5
    iget v0, v8, Lcom/android/dialer/database/CallLogQueryHandler;->mLogLimit:I

    if-ne v0, v3, :cond_6

    const/16 v0, 0x3e8

    .line 262
    :cond_6
    invoke-virtual {v1}, Ljava/lang/StringBuilder;->length()I

    move-result v3

    if-lez v3, :cond_7

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    goto :goto_2

    :cond_7
    const/4 v1, 0x0

    :goto_2
    move-object v5, v1

    iget-object v1, v8, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 264
    invoke-static {v1}, Lcom/android/dialer/telecom/TelecomUtil;->getCallLogUri(Landroid/content/Context;)Landroid/net/Uri;

    move-result-object v1

    .line 265
    invoke-virtual {v1}, Landroid/net/Uri;->buildUpon()Landroid/net/Uri$Builder;

    move-result-object v1

    const-string v3, "limit"

    .line 266
    invoke-static {v0}, Ljava/lang/Integer;->toString(I)Ljava/lang/String;

    move-result-object v0

    invoke-virtual {v1, v3, v0}, Landroid/net/Uri$Builder;->appendQueryParameter(Ljava/lang/String;Ljava/lang/String;)Landroid/net/Uri$Builder;

    move-result-object v0

    .line 267
    invoke-virtual {v0}, Landroid/net/Uri$Builder;->build()Landroid/net/Uri;

    move-result-object v3

    const/4 v4, 0x0

    .line 272
    invoke-static {}, Lcom/android/dialer/phonenumbercache/CallLogQuery;->getProjection()[Ljava/lang/String;

    move-result-object v6

    .line 274
    invoke-interface {v2}, Ljava/util/List;->size()I

    move-result v0

    new-array v0, v0, [Ljava/lang/String;

    invoke-interface {v2, v0}, Ljava/util/List;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v0

    move-object v7, v0

    check-cast v7, [Ljava/lang/String;

    const-string v9, "date DESC"

    move-object v0, p0

    move v1, p1

    move-object v2, v4

    move-object v4, v6

    move-object v6, v7

    move-object v7, v9

    .line 268
    invoke-virtual/range {v0 .. v7}, Lcom/android/dialer/database/CallLogQueryHandler;->startQuery(ILjava/lang/Object;Landroid/net/Uri;[Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Ljava/lang/String;)V

    return-void
.end method

.method private getUnreadMissedCallsQuery()Ljava/lang/String;
    .locals 1

    const-string v0, "is_read = 0 OR is_read IS NULL AND type = 3"

    return-object v0
.end method

.method private updateAdapterData(Landroid/database/Cursor;)Z
    .locals 1

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mListener:Ljava/lang/ref/WeakReference;

    .line 380
    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/dialer/database/CallLogQueryHandler$Listener;

    if-eqz v0, :cond_0

    .line 382
    invoke-interface {v0, p1}, Lcom/android/dialer/database/CallLogQueryHandler$Listener;->onCallsFetched(Landroid/database/Cursor;)Z

    move-result p1

    return p1

    :cond_0
    const/4 p1, 0x0

    return p1
.end method

.method private updateMissedCallsUnreadCount(Landroid/database/Cursor;)V
    .locals 1

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mListener:Ljava/lang/ref/WeakReference;

    .line 413
    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/dialer/database/CallLogQueryHandler$Listener;

    if-eqz v0, :cond_0

    .line 415
    invoke-interface {v0, p1}, Lcom/android/dialer/database/CallLogQueryHandler$Listener;->onMissedCallsUnreadCountFetched(Landroid/database/Cursor;)V

    :cond_0
    return-void
.end method

.method private updateVoicemailStatus(Landroid/database/Cursor;)V
    .locals 1

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mListener:Ljava/lang/ref/WeakReference;

    .line 399
    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/dialer/database/CallLogQueryHandler$Listener;

    if-eqz v0, :cond_0

    .line 401
    invoke-interface {v0, p1}, Lcom/android/dialer/database/CallLogQueryHandler$Listener;->onVoicemailStatusFetched(Landroid/database/Cursor;)V

    :cond_0
    return-void
.end method

.method private updateVoicemailUnreadCount(Landroid/database/Cursor;)V
    .locals 1

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mListener:Ljava/lang/ref/WeakReference;

    .line 406
    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/dialer/database/CallLogQueryHandler$Listener;

    if-eqz v0, :cond_0

    .line 408
    invoke-interface {v0, p1}, Lcom/android/dialer/database/CallLogQueryHandler$Listener;->onVoicemailUnreadCountFetched(Landroid/database/Cursor;)V

    :cond_0
    return-void
.end method


# virtual methods
.method public clearOldCalls(IJ)V
    .locals 8

    .line 127
    new-instance v0, Ljava/lang/StringBuilder;

    const-string v1, "clearOldCalls: "

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v1, ", "

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0, p2, p3}, Ljava/lang/StringBuilder;->append(J)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    const/4 v1, 0x0

    new-array v1, v1, [Ljava/lang/Object;

    const-string v2, "CallLogQueryHandler"

    invoke-static {v2, v0, v1}, Lcom/android/dialer/common/LogUtil;->i(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    .line 128
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    .line 129
    new-instance v1, Ljava/util/ArrayList;

    invoke-direct {v1}, Ljava/util/ArrayList;-><init>()V

    const/4 v2, -0x1

    if-le p1, v2, :cond_0

    const-string v2, "(type = ?)"

    .line 132
    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 133
    invoke-static {p1}, Ljava/lang/Integer;->toString(I)Ljava/lang/String;

    move-result-object p1

    invoke-interface {v1, p1}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    goto :goto_0

    :cond_0
    const-string p1, "NOT (type = 4)"

    .line 135
    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    :goto_0
    const-string p1, " AND (date < ?)"

    .line 139
    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 140
    invoke-static {p2, p3}, Ljava/lang/Long;->toString(J)Ljava/lang/String;

    move-result-object p1

    invoke-interface {v1, p1}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    iget-object p1, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 143
    invoke-static {p1}, Lcom/android/dialer/telecom/TelecomUtil;->getCallLogUri(Landroid/content/Context;)Landroid/net/Uri;

    move-result-object v5

    const/4 v3, 0x0

    const/4 v4, 0x0

    .line 145
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v6

    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result p1

    new-array p1, p1, [Ljava/lang/String;

    invoke-interface {v1, p1}, Ljava/util/List;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object p1

    move-object v7, p1

    check-cast v7, [Ljava/lang/String;

    move-object v2, p0

    .line 144
    invoke-virtual/range {v2 .. v7}, Lcom/android/dialer/database/CallLogQueryHandler;->startDelete(ILjava/lang/Object;Landroid/net/Uri;Ljava/lang/String;[Ljava/lang/String;)V

    return-void
.end method

.method protected createHandler(Landroid/os/Looper;)Landroid/os/Handler;
    .locals 1

    .line 123
    new-instance v0, Lcom/android/dialer/database/CallLogQueryHandler$CatchingWorkerHandler;

    invoke-direct {v0, p0, p1}, Lcom/android/dialer/database/CallLogQueryHandler$CatchingWorkerHandler;-><init>(Lcom/android/dialer/database/CallLogQueryHandler;Landroid/os/Looper;)V

    return-object v0
.end method

.method public fetchCalls(I)V
    .locals 3

    const/4 v0, 0x0

    const-wide/16 v1, 0x0

    .line 164
    invoke-virtual {p0, p1, v0, v1, v2}, Lcom/android/dialer/database/CallLogQueryHandler;->fetchCalls(ILjava/lang/String;J)V

    return-void
.end method

.method public fetchCalls(ILjava/lang/String;)V
    .locals 2

    const-wide/16 v0, 0x0

    .line 168
    invoke-virtual {p0, p1, p2, v0, v1}, Lcom/android/dialer/database/CallLogQueryHandler;->fetchCalls(ILjava/lang/String;J)V

    return-void
.end method

.method public fetchCalls(ILjava/lang/String;J)V
    .locals 8

    .line 155
    invoke-direct {p0}, Lcom/android/dialer/database/CallLogQueryHandler;->cancelFetch()V

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 156
    invoke-static {v0}, Lcom/android/dialer/util/PermissionsUtil;->hasPhonePermissions(Landroid/content/Context;)Z

    move-result v0

    if-eqz v0, :cond_0

    const/16 v2, 0x36

    const/4 v4, 0x0

    move-object v1, p0

    move v3, p1

    move-object v5, p2

    move-wide v6, p3

    .line 157
    invoke-direct/range {v1 .. v7}, Lcom/android/dialer/database/CallLogQueryHandler;->fetchCalls(IIZLjava/lang/String;J)V

    goto :goto_0

    :cond_0
    const/4 p1, 0x0

    .line 159
    invoke-direct {p0, p1}, Lcom/android/dialer/database/CallLogQueryHandler;->updateAdapterData(Landroid/database/Cursor;)Z

    :goto_0
    return-void
.end method

.method public fetchMissedCallsUnreadCount()V
    .locals 9

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 333
    invoke-static {v0}, Lcom/android/dialer/util/PermissionsUtil;->hasPhonePermissions(Landroid/content/Context;)Z

    move-result v0

    if-nez v0, :cond_0

    return-void

    :cond_0
    const/16 v2, 0x3b

    const/4 v3, 0x0

    .line 337
    sget-object v4, Landroid/provider/CallLog$Calls;->CONTENT_URI:Landroid/net/Uri;

    const-string v0, "_id"

    filled-new-array {v0}, [Ljava/lang/String;

    move-result-object v5

    .line 342
    invoke-direct {p0}, Lcom/android/dialer/database/CallLogQueryHandler;->getUnreadMissedCallsQuery()Ljava/lang/String;

    move-result-object v6

    const/4 v7, 0x0

    const/4 v8, 0x0

    move-object v1, p0

    .line 337
    invoke-virtual/range {v1 .. v8}, Lcom/android/dialer/database/CallLogQueryHandler;->startQuery(ILjava/lang/Object;Landroid/net/Uri;[Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Ljava/lang/String;)V

    return-void
.end method

.method public fetchVoicemailStatus()V
    .locals 11

    .line 172
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    .line 173
    new-instance v1, Ljava/util/ArrayList;

    invoke-direct {v1}, Ljava/util/ArrayList;-><init>()V

    iget-object v2, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mBuildQueryListener:Ljava/lang/ref/WeakReference;

    .line 175
    invoke-virtual {v2}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;

    if-eqz v2, :cond_0

    iget-object v3, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 177
    invoke-interface {v2, v3, v0, v1}, Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;->appendOmtpVoicemailStatusSelectionClause(Landroid/content/Context;Ljava/lang/StringBuilder;Ljava/util/List;)V

    :cond_0
    iget-object v2, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 180
    invoke-static {v2}, Lcom/android/dialer/telecom/TelecomUtil;->hasReadWriteVoicemailPermissions(Landroid/content/Context;)Z

    move-result v2

    if-eqz v2, :cond_1

    const/16 v4, 0x39

    const/4 v5, 0x0

    .line 181
    sget-object v6, Landroid/provider/VoicemailContract$Status;->CONTENT_URI:Landroid/net/Uri;

    .line 185
    invoke-static {}, Lcom/android/dialer/database/VoicemailStatusQuery;->getProjection()[Ljava/lang/String;

    move-result-object v7

    .line 186
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v8

    .line 187
    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v0

    new-array v0, v0, [Ljava/lang/String;

    invoke-interface {v1, v0}, Ljava/util/List;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v0

    move-object v9, v0

    check-cast v9, [Ljava/lang/String;

    const/4 v10, 0x0

    move-object v3, p0

    .line 181
    invoke-virtual/range {v3 .. v10}, Lcom/android/dialer/database/CallLogQueryHandler;->startQuery(ILjava/lang/Object;Landroid/net/Uri;[Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Ljava/lang/String;)V

    :cond_1
    return-void
.end method

.method public fetchVoicemailUnreadCount()V
    .locals 12

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 193
    invoke-static {v0}, Lcom/android/dialer/telecom/TelecomUtil;->hasReadWriteVoicemailPermissions(Landroid/content/Context;)Z

    move-result v0

    if-eqz v0, :cond_1

    .line 195
    new-instance v0, Ljava/lang/StringBuilder;

    const-string v1, "is_read=0 AND deleted=0 "

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    .line 197
    new-instance v1, Ljava/util/ArrayList;

    invoke-direct {v1}, Ljava/util/ArrayList;-><init>()V

    iget-object v2, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mBuildQueryListener:Ljava/lang/ref/WeakReference;

    .line 199
    invoke-virtual {v2}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;

    if-eqz v2, :cond_0

    iget-object v3, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 201
    invoke-interface {v2, v3, v0, v1}, Lcom/android/dialer/database/CallLogQueryHandler$BuildQueryListener;->appendOmtpVoicemailSelectionClause(Landroid/content/Context;Ljava/lang/StringBuilder;Ljava/util/List;)V

    :cond_0
    const/16 v5, 0x3a

    const/4 v6, 0x0

    .line 204
    sget-object v7, Landroid/provider/VoicemailContract$Voicemails;->CONTENT_URI:Landroid/net/Uri;

    const-string v2, "_id"

    filled-new-array {v2}, [Ljava/lang/String;

    move-result-object v8

    .line 209
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v9

    .line 210
    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v0

    new-array v0, v0, [Ljava/lang/String;

    invoke-interface {v1, v0}, Ljava/util/List;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v0

    move-object v10, v0

    check-cast v10, [Ljava/lang/String;

    const/4 v11, 0x0

    move-object v4, p0

    .line 204
    invoke-virtual/range {v4 .. v11}, Lcom/android/dialer/database/CallLogQueryHandler;->startQuery(ILjava/lang/Object;Landroid/net/Uri;[Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Ljava/lang/String;)V

    :cond_1
    return-void
.end method

.method public markMissedCallsAsRead()V
    .locals 8

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 313
    invoke-static {v0}, Lcom/android/dialer/util/PermissionsUtil;->hasPhonePermissions(Landroid/content/Context;)Z

    move-result v0

    if-nez v0, :cond_0

    return-void

    .line 317
    :cond_0
    new-instance v5, Landroid/content/ContentValues;

    const/4 v0, 0x1

    invoke-direct {v5, v0}, Landroid/content/ContentValues;-><init>(I)V

    const-string v0, "is_read"

    const-string v1, "1"

    .line 318
    invoke-virtual {v5, v0, v1}, Landroid/content/ContentValues;->put(Ljava/lang/String;Ljava/lang/String;)V

    const/16 v2, 0x38

    const/4 v3, 0x0

    .line 320
    sget-object v4, Landroid/provider/CallLog$Calls;->CONTENT_URI:Landroid/net/Uri;

    .line 325
    invoke-direct {p0}, Lcom/android/dialer/database/CallLogQueryHandler;->getUnreadMissedCallsQuery()Ljava/lang/String;

    move-result-object v6

    const/4 v7, 0x0

    move-object v1, p0

    .line 320
    invoke-virtual/range {v1 .. v7}, Lcom/android/dialer/database/CallLogQueryHandler;->startUpdate(ILjava/lang/Object;Landroid/net/Uri;Landroid/content/ContentValues;Ljava/lang/String;[Ljava/lang/String;)V

    return-void
.end method

.method public markNewCallsAsOld()V
    .locals 8

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 289
    invoke-static {v0}, Lcom/android/dialer/util/PermissionsUtil;->hasPhonePermissions(Landroid/content/Context;)Z

    move-result v0

    if-nez v0, :cond_0

    return-void

    .line 297
    :cond_0
    new-instance v5, Landroid/content/ContentValues;

    const/4 v0, 0x1

    invoke-direct {v5, v0}, Landroid/content/ContentValues;-><init>(I)V

    const-string v0, "new"

    const-string v1, "0"

    .line 298
    invoke-virtual {v5, v0, v1}, Landroid/content/ContentValues;->put(Ljava/lang/String;Ljava/lang/String;)V

    const/16 v2, 0x37

    const/4 v3, 0x0

    iget-object v0, p0, Lcom/android/dialer/database/CallLogQueryHandler;->mContext:Landroid/content/Context;

    .line 303
    invoke-static {v0}, Lcom/android/dialer/telecom/TelecomUtil;->getCallLogUri(Landroid/content/Context;)Landroid/net/Uri;

    move-result-object v4

    const-string v6, "new = 1"

    const/4 v7, 0x0

    move-object v1, p0

    .line 300
    invoke-virtual/range {v1 .. v7}, Lcom/android/dialer/database/CallLogQueryHandler;->startUpdate(ILjava/lang/Object;Landroid/net/Uri;Landroid/content/ContentValues;Ljava/lang/String;[Ljava/lang/String;)V

    return-void
.end method

.method protected declared-synchronized onNotNullableQueryComplete(ILjava/lang/Object;Landroid/database/Cursor;)V
    .locals 2

    const-string p2, "unknown query completed: ignoring: "

    monitor-enter p0

    if-nez p3, :cond_0

    .line 350
    monitor-exit p0

    return-void

    :cond_0
    const/16 v0, 0x36

    if-ne p1, v0, :cond_1

    .line 354
    :try_start_0
    invoke-direct {p0, p3}, Lcom/android/dialer/database/CallLogQueryHandler;->updateAdapterData(Landroid/database/Cursor;)Z

    move-result p1

    if-eqz p1, :cond_5

    const/4 p3, 0x0

    goto :goto_0

    :catchall_0
    move-exception p1

    goto :goto_2

    :cond_1
    const/16 v0, 0x39

    if-ne p1, v0, :cond_2

    .line 358
    invoke-direct {p0, p3}, Lcom/android/dialer/database/CallLogQueryHandler;->updateVoicemailStatus(Landroid/database/Cursor;)V

    goto :goto_0

    :cond_2
    const/16 v0, 0x3a

    if-ne p1, v0, :cond_3

    .line 360
    invoke-direct {p0, p3}, Lcom/android/dialer/database/CallLogQueryHandler;->updateVoicemailUnreadCount(Landroid/database/Cursor;)V

    goto :goto_0

    :cond_3
    const/16 v0, 0x3b

    if-ne p1, v0, :cond_4

    .line 362
    invoke-direct {p0, p3}, Lcom/android/dialer/database/CallLogQueryHandler;->updateMissedCallsUnreadCount(Landroid/database/Cursor;)V

    goto :goto_0

    :cond_4
    const-string v0, "CallLogQueryHandler.onNotNullableQueryComplete"

    .line 364
    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1, p2}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v1, p1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    const/4 p2, 0x0

    new-array p2, p2, [Ljava/lang/Object;

    invoke-static {v0, p1, p2}, Lcom/android/dialer/common/LogUtil;->w(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    :cond_5
    :goto_0
    if-eqz p3, :cond_6

    .line 370
    :try_start_1
    invoke-interface {p3}, Landroid/database/Cursor;->close()V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_1

    goto :goto_1

    :catchall_1
    move-exception p1

    goto :goto_3

    .line 373
    :cond_6
    :goto_1
    monitor-exit p0

    return-void

    :goto_2
    if-eqz p3, :cond_7

    .line 370
    :try_start_2
    invoke-interface {p3}, Landroid/database/Cursor;->close()V

    .line 372
    :cond_7
    throw p1
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_1

    :goto_3
    monitor-exit p0

    throw p1
.end method
